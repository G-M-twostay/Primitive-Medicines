...
namespace PrimMed.Patches
{
    ...
    static class _ItemComponent
    {
        ...
        public static void _ApplyStatusEffects(ItemComponent __instance, in ActionType type, Character character, Limb targetLimb, Character user)
        {
            if (__instance.Item.HasTag("syringe"))
                switch (type)
                {
                    ...
                    case ActionType.OnSuccess:
                        ...
                        {
                            float pierceAmount;
                            if (ReferenceEquals(user, character))
