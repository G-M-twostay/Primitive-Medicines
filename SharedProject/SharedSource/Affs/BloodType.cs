...
namespace PrimMed.Affs{
    ...
    class BloodType:Affliction{
        ...
        internal static BloodType chooseType(){
            int r=Rand.Int(RankedTypes[RankedTypes.Length-1].Item2,Rand.RandSync.ServerAndClient);
            int index=Array.BinarySearch(RankedTypes,(null,(ushort)r),Comparer<(string,ushort)>.Create((a,b)=>a.Item2.CompareTo(b.Item2)));
            if(index<0)
                index=~index;
